...
`geodata/` 目录放两个库,程序自动加载,精确到「国家/省/市 + 运营商」,国内外通用:

| 文件 | 作用 | 获取 |
|---|---|---|
| `ip2asn-v4.tsv` | 全球运营商(国内识别为电信/联通/移动) | 已内置 |
| `dbip-city-lite-*.mmdb` | 全球国家 + 省 + 市 | 需自行下载,放进 geodata/ |

城市库下载(免费、无需注册):https://db-ip.com/db/download/ip-to-city-lite
下载后解压,把 `.mmdb` 文件放到 `geodata/` 即可(程序自动识别该目录下任意 `.mmdb`)。

> 不放城市库也能跑:此时省/市为空,仍可显示国家 + 运营商。
